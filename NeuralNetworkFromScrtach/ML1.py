...
'''Part 6 of Video - Softmax Activation
# As all nodes are getting the same inputs, the inputs is still only 1 array of the same values
import numpy as np
np.random.seed(0)
...
dense1 = Layer_Dense(2,3) # input is 2 here because data (spiral data) is ony x, y values so 2 dimensions - the 3 is the number of nodes
Activation1 = Activation_ReLU()
...
print(Activation2.output[:5])# just print first 5'''

'''Part 7 of Video - Loss Calculation'''
# As all nodes are getting the same inputs, the inputs is still only 1 array of the same values
import numpy as np
np.random.seed(0)
...
lossvalue = lossfunction.calculate(Activation2.output, y)
